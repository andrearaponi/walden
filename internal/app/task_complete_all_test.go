...
func TestRunTaskCompleteAllPrintsHumanReadableSuccess(t *testing.T) {
	root := t.TempDir()
...
`)
	...
	commandRunner = testutil.NewFakeRunner(
	...
	exitCode := Run([]string{"task", "complete-all", "todo-app-demo"}, &stdout, &stderr)

	if exitCode != 0 {
		t.Fatalf("expected batch complete to succeed, got %d and stderr %q", exitCode, stderr.String())
	}

	rendered := stdout.String()
	for _, want := range []string{
		"Summary: batch task completion finished for todo-app-demo",
		"Changed files:",
...
func TestRunTaskCompleteAllPrintsJSONFailureWithPartialProgress(t *testing.T) {
	root := t.TempDir()
	writeStatusFeatureFile(t, root, "todo-app-demo", "requirements.md", `---
status: approved
approved_at: 2026-03-21T14:00:00Z
...
`)
	...
	exitCode := Run([]string{"task", "complete-all", "todo-app-demo", "--json"}, &stdout, &stderr)

	if exitCode != 1 {
		t.Fatalf("expected batch complete to fail, got %d", exitCode)
	}
	if stderr.Len() != 0 {
	...
	if !strings.Contains(result.Summary, `verification failed for task "1.2"`) {
	...
	if len(result.AutoCompleted) != 0 {
		t.Fatalf("expected no auto-completed parent tasks, got %#v", result.AutoCompleted)
	}
	if result.NextAction != "Fix failing proof for task 1.2 and rerun batch completion" {
