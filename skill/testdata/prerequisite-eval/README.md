...
Four candidate-only synthetic cases for the v0.10.3 guide, which **never installs the CLI**: compatible CLI on PATH, compatible CLI only under `$HOME/.local/bin`, missing CLI followed by an explicit "install it for me" request, and native/Skills CLI ownership overlap. Five user turns in total.
...
- points-to-official-install: the missing-CLI answer names the repository README / GitHub releases as the place to install from;
...
Limits: four sessions, at most five turns, at most 180 active seconds and a requested $0.50 client cap per session, $1 aggregate. No retry, no extra model preflight. A failed or not-run case blocks acceptance. The v0.10.2 `bootstrap-eval` set is retired with the capability it measured; its observations are retained privately, not relabeled.
